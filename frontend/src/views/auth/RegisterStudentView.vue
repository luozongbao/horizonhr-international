...
const rules = computed<FormRules>(() => ({
  ...
  nationality: [{ required: true, message: t('validation.nationalityRequired'), trigger: 'change' }],
  pdpa_consent: [
    {
...
const nationalities = [
  'Thai', 'Myanmar', 'Vietnamese', 'Malaysian', 'Indonesian', 'Filipino',
  'Cambodian', 'Laotian', 'Singaporean', 'Bangladeshi', 'Indian', 'Pakistani',
  'Nepali', 'Sri Lankan', 'Japanese', 'South Korean', 'Mongolian', 'Other',
]
</script>
...
<template>
  <div class="auth-container">
    <aside class="auth-left">
      ...
      <h1 class="left-title">Start Your Journey to Study in China</h1>
      <p class="left-sub">Create a student account to access scholarship opportunities, university applications, and career connections.</p>
      <div class="features">
        <div class="feature">
          <div class="feature-icon">🎓</div>
          <div><p class="feature-title">University Access</p><p class="feature-desc">Apply to 50+ partner universities</p></div>
        </div>
        <div class="feature">
          <div class="feature-icon">📄</div>
          <div><p class="feature-title">Smart Resume</p><p class="feature-desc">AI-powered resume builder in 3 languages</p></div>
        </div>
        <div class="feature">
          <div class="feature-icon">💼</div>
          <div><p class="feature-title">Career Placement</p><p class="feature-desc">Connect with top Chinese enterprises</p></div>
        </div>
      </div>
    </aside>
    ...
    <section class="auth-right">
      <div class="form-wrap">
        <div class="auth-tabs">
          ...
          <router-link to="/register/student" class="auth-tab active">{{ t('auth.register') }}</router-link>
        </div>
        ...
        <template v-else>
          <h2 class="form-title">{{ t('auth.registerStudent') }}</h2>
          <p class="form-sub">
          ...
          <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
            <div class="form-grid">
              <el-form-item :label="t('auth.name')" prop="name" class="full">
                <el-input v-model="form.name" size="large" :placeholder="t('auth.name')" />
              </el-form-item>
              ...
              <el-form-item :label="t('auth.nationality')" prop="nationality" class="full">
                <el-select v-model="form.nationality" size="large" :placeholder="t('auth.nationality')" style="width:100%" filterable>
          ...
          <p class="switch-link">
            Looking to hire?
            <router-link to="/register/enterprise">{{ t('auth.switchToEnterprise') }}</router-link>
          </p>
        </template>
      </div>
    </section>
